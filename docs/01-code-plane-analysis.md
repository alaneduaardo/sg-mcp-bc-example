...
Since the December 2025 split — Amp spun out as an independent company; Sourcegraph refocused as enterprise code intelligence — the strategic position of Code Plane has sharpened. With Amp gone, Sourcegraph's own product surface no longer fields an inner-loop coding assistant; it positions itself instead as the **vendor-neutral infrastructure layer that agents from any vendor consume**: retrieval standardized through MCP, action at scale through Batch Changes.
...
**Implemented seed (what v1 already surfaces).** `bc_preview` computes the inputs the predictive layers consume, at the one moment the dry run already pays for them. Resolution runs with `count:all`, so the resolved-repo set and changeset count are *complete* rather than a ranked sample — the blast-radius **surface** of layer 1 (the *count* and identity of what a run touches), not yet its **ordering** (leaf-vs-hub scoring remains the hard, unsolved part). From that count it derives `estimated_phases` — `ceil(changesets / batch_size)` at the default staged-rollout batch — a first-order foreshadow of layer 3's staged rollout: a phase *count*, not a blast-radius-ordered, CI-gated schedule. The point is narrow and honest: the dry run is the natural place to compute these, before anything publishes — the seed is real, the layer is not yet built.
